...
# ── test every import individually and show exact errors ──────────────────────
_ok = True
...
try:
    ...
    st.error(f"❌ rca_engine import failed: {e}")
    st.code(traceback.format_exc())
    _ok = False
...
try:
    ...
    st.error(f"❌ report_generator import failed: {e}")
    st.code(traceback.format_exc())
    _ok = False

if not _ok:
    st.warning(f"Python {sys.version} | pandas {pd.__version__ if 'pd' in dir() else 'not loaded'} | numpy {np.__version__ if 'np' in dir() else 'not loaded'}")
    st.stop()
...
def kpi_row(summary):
    c = st.columns(6)
    c[0].metric("Loss vs L7D (orders/day)",  n(summary.get("total_loss_vs_l7d")),  delta="orders/day", delta_color="inverse")
    c[1].metric("Loss vs L30D (orders/day)", n(summary.get("total_loss_vs_l30d")), delta="orders/day", delta_color="inverse")
    c[2].metric("L1D Orders (yesterday)",    n(summary.get("total_l1d_orders")))
    c[3].metric("Monthly Impact",            n(summary.get("monthly_impact")),      delta="orders", delta_color="inverse")
    c[4].metric("Avg Health Score",          f"{n(summary.get('avg_health_score'))} /100")
    c[5].metric("On Velocity Cliff",         n(summary.get("velocity_cliff_count")), delta="crashed", delta_color="inverse")
...
def show_actionables(actions, df_filtered):
    ...
    if st.button("🏷️  Generate tagged download", key="gen_tag"):
        ...
        st.download_button("⬇️  Download tagged CSV",
            ...
            file_name=f"RCA_tagged_{date.today()}.csv",
            mime="text/csv", key="dl_tag")
...
def show_innovative(innov):
    tabs = st.tabs(["⚡ Compounding","🌊 Cascade Risk","📉 Velocity Cliff","🧊 Stranded"])
    with tabs[0]:
        ci = innov.get("compounding_issues", {})
        st.metric("Listings with 2+ problems", ci.get("count",0),
                  delta=f"−{n(ci.get('total_loss_vs_l7d'))} orders/day", delta_color="inverse")
        if ci.get("listings"):
            st.dataframe(pd.DataFrame([{
                "Listing": i["product_title"], "Seller": i["display_name"],
                "Owner": i.get("name",""), "Problems": i["problems"],
                "#Issues": i["issue_count"], "Loss vs L7D": i["loss_vs_l7d"],
    ...
    with tabs[1]:
        ...
        if sellers:
            st.dataframe(pd.DataFrame([{
                ...
                "Orders/Day": s["orders_per_day"], "OOS": s["oos_count"],
                "Risk Score": s["risk_score"]} for s in sellers]),
                use_container_width=True, hide_index=True)
    with tabs[2]:
        vc = innov.get("velocity_cliff_listings", {})
        st.metric("On velocity cliff", vc.get("count", 0))
        if vc.get("listings"):
            st.dataframe(pd.DataFrame([{
                "Listing": i["product_title"], "Seller": i["display_name"],
                "Owner": i.get("name",""), "L1D": i["rate_l1d"],
                "L30D": i["rate_l30d"], "Health": i["health_score"],
            } for i in vc["listings"]]), use_container_width=True, hide_index=True)
    with tabs[3]:
        ...
        if si.get("listings"):
            st.dataframe(pd.DataFrame([{
                "Listing": i["product_title"], "Seller": i["display_name"],
                "ATP": i["yesterday_atp"], "ASP": inr(i["l7d_asp"]),
            } for i in si["listings"]]), use_container_width=True, hide_index=True)
...
def show_recommendations(recs):
    ...
    if um:
        st.dataframe(pd.DataFrame([{
            "Seller": s["seller"], "L30D Orders": n(s["l30d_orders"]),
            "ASP": inr(s["avg_asp"]), "Health": s["health"],
            "Growth": pct(s["growth_pct"]), "Trend": s["trend"],
        } for s in um]), use_container_width=True, hide_index=True)
    else:
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.dim_values:
            ...
            val_choice = st.selectbox("Value", values, key="val_choice")
    ...
    if load_btn:
        ...
        if not raw_file:
            ...
            st.warning(f"Files visible in folder ({len(all_files)}): {[f['name'] for f in all_files]}")
            st.stop()
        ...
        st.success(f"Loaded {len(df_raw):,} rows. Dimensions: {', '.join(dim_values.keys())}")
        st.rerun()
